...
static const unsigned char PROGMEM Lvl_fault_Icon[] = {0x0f,0xe0,0x10,0x10,0x20,0x08,0x40,0x04,0x88,0x22,0x84,0x42,0x82,0x82,0x81,0x02,0x82,0x82,0x84,0x42,0x88,0x22,0x40,0x04,0x20,0x08,0x10,0x10,0x0f,0xe0,0x00,0x00};
...
void displayLightAndPresence(SystemData* data) {
    ...
    data->oledDisplay->SetdisplayData(61, 41, data->PirPresenceDetected ? "YES" : "NO");

    data->oledDisplay->SetdisplayData(100, 14, "Lamp");
    data->oledDisplay->DrawIcon(104, 23, LampState ? Lamp_On_Icon : Lamp_Off_Icon, 13, 16);
    data->oledDisplay->SetdisplayData(102, 41, LampState ? "ON" : "OFF");
...
void displayWaterLevelAndPump(SystemData* data) {
    ...
    data->oledDisplay->SetdisplayData(1, 14, "Level Sensor");

    if (levelValue >= SENSOR_LVL_OPENCKT_V) {
        data->oledDisplay->DrawIcon(4, 28, Lvl_fault_Icon, 15, 16);
        data->oledDisplay->SetdisplayData(23, 32, "OPEN");
    } else if (levelValue <= SENSOR_LVL_STG_V + SENSOR_LVL_THRESHOLD_V) {
        data->oledDisplay->DrawIcon(4, 28, Lvl_fault_Icon, 15, 16);
        data->oledDisplay->SetdisplayData(23, 32, "SHORT");
    } else if (levelValue >= SENSOR_LVL_OPENCKT_V - SENSOR_LVL_THRESHOLD_V) {
        data->oledDisplay->setTextProperties(2, SSD1306_WHITE);
        data->oledDisplay->DrawIcon(0, 25, Lvl_Icon, 24, 20);
        data->oledDisplay->SetdisplayData(31, 29, "100%");
    } else {
        data->oledDisplay->DrawIcon(0, 25, Lvl_Icon, 24, 20);
        data->oledDisplay->setTextProperties(2, SSD1306_WHITE);
        data->oledDisplay->SetdisplayData(31, 29, data->levelPercentage);
        data->oledDisplay->SetdisplayData(67, 29, "%");
    }

    data->oledDisplay->setTextProperties(1, SSD1306_WHITE);

    data->oledDisplay->SetdisplayData(95, 14, "Pump");
